...
export const updateExpense = async (expenseId, updatedExpense, userId) => {
  checkSupabaseConfig();

  let receiptImageUrl = updatedExpense.receipt_image;

  // Upload new receipt image if provided
  if (updatedExpense.receiptImage) {
    receiptImageUrl = await uploadReceiptImage(updatedExpense.receiptImage, userId);
  }
