...
export const sendPasswordResetEmail = async (email) => {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      // Use proper Expo deep linking
      const Linking = require('expo-linking');
      const redirectTo = Linking.createURL('reset-password', {
        scheme: 'mynaaapp', // matches app.json scheme
      });
      ...
      if (error) {
        ...
        if (error.message?.includes('504') || error.message?.includes('timeout') || error.status === 504) {
          lastError = error;
          console.log(`Password reset attempt ${attempt} failed with 504, retrying...`);
...
export const updatePasswordWithToken = async (token, newPassword) => {
  try {
    ...
    // For password reset, we need to verify the token and update password
    ...
    // The verifyOtp call should have established a session
    // Check if we have a valid session
    const { data: { session }, error: sessionError } = await supabase.auth.getSession();

    if (sessionError || !session) {
      throw new Error('Failed to establish session after token verification');
    }
